...
class TableTest {
    ...
    @Test
    // ADDED - 1
    void placeCard_removeCard() throws InterruptedException {
        table.placeCard(8, 2);

        table.removeCard(2);

        // int expected = Player.empty; // indicates empty slot
    ...
    @Test
    // ADDED - 2
    void removeCard_countCards() throws InterruptedException {
        table.placeCard(8, 2);

        int countCards = table.countCards();

        int expectedCount = (countCards - 1);

        table.removeCard(2);

        assertEquals(expectedCount, table.countCards());
    }
